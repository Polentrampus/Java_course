...
public class GetBookingByIdAction extends BaseAction {
    ...
    private void printBookingDetails(Bookings booking) throws SQLException {
        ...
        System.out.println("ФИО: " + clientService.findById(booking.getClient().getId()).get());
        System.out.println("Дата рождения: " +  clientService.findById(booking.getClient().getId()).get().getDateOfBirth());

        System.out.println("\n--- ИНФОРМАЦИЯ О КОМНАТЕ ---");
        System.out.println("Номер: " + booking.getRoom());
        System.out.println("Описание: " + roomService.findById(booking.getRoom().getId()).get());
        ...
        System.out.println("Стоимость проживания: $" +
                roomService.findById(booking.getRoom().getId()).get().getPrice().multiply(java.math.BigDecimal.valueOf(days)));
