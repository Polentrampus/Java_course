...
public class AdminController {
    ...
    public void settle(Client client, LocalDate checkInDate, int day ) {
        if (roomService.listAvailableRoomsByDate(RoomFilter.ID, checkInDate).isEmpty()) {
            System.out.printf("Не удалось заселить клиента %s, нет свободных комнат к определенной дате\n", client.toString());
            return;
        }
        int idRoom = roomService.listAvailableRoomsByDate(RoomFilter.ID, checkInDate).getFirst().getNumber();
        hotel.getClientMap().get().put(client.getId(), client);
        Bookings bookings = bookingService.createBooking(new CreateBookingRequest(client.getId(),
                idRoom, checkInDate, checkInDate.plusDays(day))).get();

        System.out.printf("Клиента %s поселили\n", hotel.getClientMap().get().get(client.getId()).toString() +
                " в номер:" + bookings.getRoom().getNumber() + "бронь действительна с " + checkInDate +
                " до " + checkInDate.plusDays(day) + "\n");
    }

    public void settle(Client client) {
        if (roomService.listAvailableRooms(RoomFilter.ID).isEmpty()) {
            System.out.printf("Не удалось заселить клиента %s, нет свободных комнат\n", client.toString());
            return;
        }
        int idRoom = roomService.listAvailableRooms(RoomFilter.ID).getFirst().getNumber();
        hotel.getClientMap().get().put(client.getId(), client);

        Bookings bookings = bookingService.createBooking(new CreateBookingRequest(client.getId(),
                idRoom, LocalDate.now(), LocalDate.now().plusDays(1))).get();

        System.out.printf("Клиента %s поселили\n", hotel.getClientMap().get().get(client.getId()).toString() +
                " в номер:" + bookings.getRoom().getNumber()  + "\n");
    }

    public void givOutCheck(int idClient) {
        double sum = bookingService.givOutCheck(idClient);
    ...
    public void evict(int idClient) {
        ...
        Bookings bookings = bookingService.getBookingByIdClient(idClient).get();
        int roomId = bookings.getRoom().getNumber();
