...
public class UpdateBookingAction extends BaseAction {
    ...
    public void execute() {
        try {
            ...
            System.out.println("\n--- ОБНОВЛЕНИЕ КЛИЕНТА ---");
            System.out.println("Текущий клиент: " + clientService.findById(booking.getClient()).get());
            ...
            int clientId = clientInput.isEmpty() ?
                    booking.getClient() : Integer.parseInt(clientInput);
            ...
            int roomId = roomInput.isEmpty() ? booking.getRoom() : Integer.parseInt(roomInput);
