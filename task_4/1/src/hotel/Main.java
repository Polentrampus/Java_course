...
public class Main {
    public static void main(String[] args) {
        ...
        adminController.requestListRoom(RoomFilter.CAPACITY);
        adminController.getListAvailableRooms(RoomFilter.PRICE);
        adminController.getInfoAboutClientDatabase(ClientFilter.DATECHECKUP);
        adminController.requestLastThreeClient();
        adminController.requestListServicesClient(ServicesFilter.PRICE, 1);
        adminController.requestListServices();
        adminController.addService("TSAR", "Вас покормят наши сотрудники с ложки", 1000000);
        adminController.requestListServices();
        adminController.changePriceService("TSAR", 50000);
        adminController.requestListRoomAndPrice(RoomFilter.ID);
        adminController.addClientServices(2, hotel.getServicesByKeys("CONCIERGE", "SPA"));
